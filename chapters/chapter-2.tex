...
In this equation, $\mathbf{x}$ represents the real data, and $\mathbf{z}$ represents a random latent vector sampled from a prior distribution $p_{\mathbf{z}}$. The first term represents the discriminator successfully recognizing real geological data, while the second term represents the discriminator successfully catching fake data generated by $\mathbf{G}(\mathbf{z})$. During training, the discriminator seeks to maximize this overall value function to perfect its detection abilities, while the generator seeks to minimize it to successfully fool the discriminator. By successfully fooling the discriminator, the generator ultimately learns to produce high-fidelity geological realizations without requiring any explicit, hard-coded spatial rules. On overview of the the training process is provided in figure \ref{fig:GAN_architecture}.

\subsubsection{GAN Inversion}
Figure \ref{fig:GAN_architecture} illustrates the generation of high-fidelity, unconstrained realizations, which is a powerful capability; however, practical reservoir modelling requires these models to honor sparse physical constraints like hard well data. Hard well data constraints are typically honored through an optimization technique known as GAN inversion. GAN inversion operates strictly within the model's latent space after the training phase is complete. Completing the training phase allows the algorithm to iteratively search and optimize this latent space to identify a specific latent vector z. This specific latent vector z generates a 3D volume perfectly matching the sparse well data. Matching the sparse well data securely conditions the generative model to real-world subsurface observations.

\subsubsection{Wasserstein Metric for Loss and Validation}
Real-world subsurface observations used to condition the realizations must be rigorously evaluated to ensure geological realism and consistency. This consistency in high-dimensional 3D spatial data is historically computationally difficult to evaluate due to the curse of dimensionality. The curse of dimensionality can be mitigated by utilizing the Wasserstein distance as a robust validation metric and training loss function. This training loss function calculates the optimal transport cost, or Earth Mover's Distance, between the true data probability distribution and the generated probability distribution. The generated probability distribution's deviation from the true distribution is mathematically defined by the basic Wasserstein formula:

\begin{equation}
    \mathcal{W}(P_r, P_g) = \inf_{\gamma \sim \Pi(P_r, P_g)} \mathbb{E}_{(x,y) \sim \gamma} [\|x - y\|]
\end{equation}

This basic formula calculates the minimum cost required to transform the generated distribution Pg​ into the real distribution Pr​. The real distribution Pr​ is approximated efficiently through this calculation, allowing the network to converge on highly realistic, non-linear structures even when exact pixel overlap is minimal.

\subsubsection{Mean Sliced Wasserstein Distance}
Minimal exact pixel overlap across massive 3D grid volumes makes direct mathematical comparison highly inefficient, necessitating refined validation metrics such as the Mean Sliced Wasserstein Distance (MSWD). MSWD bypasses this high dimensionality by mathematically projecting the 3D facies distributions onto multiple 1D slices. These 1D slices provide a highly efficient framework for distance comparison. This distance comparison ultimately validates whether the newly generated geomodels remain geologically consistent with the original training data.