...
While generating high-fidelity, unconstrained realizations is a powerful capability, practical reservoir modelling requires these models to honor sparse physical constraints, such as hard well data. Honoring this hard data is typically achieved through an optimization technique known as GAN inversion. Instead of altering the neural network architecture itself to accommodate well logs, GAN inversion operates strictly within the model's latent space after the training phase is complete. By iteratively searching and optimizing this latent space, the algorithm identifies a specific latent vector $\mathbf{z}$ that generates a 3D volume perfectly matching the sparse well data~\parencite{rongier2025atowards}. Identifying this optimal vector allows engineers to securely condition the generative model to real-world subsurface observations.

Once these conditioned realizations are generated, their geological realism and consistency must be rigorously evaluated. Evaluating and validating high-dimensional 3D spatial data is historically computationally difficult due to the curse of dimensionality. To overcome this computational difficulty, recent geomodelling workflows utilize refined validation metrics such as the Mean Sliced Wasserstein Distance (MSWD)~\parencite{bhavsar2024stable}. Rather than comparing massive 3D grid volumes directly, MSWD bypasses the high dimensionality by mathematically projecting the 3D facies distributions onto multiple 1D slices. Comparing the distances between these 1D slices provides a highly efficient and robust metric to validate whether the newly generated geomodels remain geologically consistent with the original training data.

Previous studies have demonstrated that \glspl{gan} can generate geologically plausible realizations while remaining training-stable~\parencite{rongier2025atowards,bhavsar2024stable}. Crucially,~\textcite{rongier2025btowards} showed that the model's latent space can be inverted and optimized to honor hard constraints from well data.The inversion makes it oissible for the model to be used in real-world workflows, whereby acquired well data is an important data source. 
