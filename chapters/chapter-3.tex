...
The primary real-world constraints for this project are derived from the physical well logs of the DEL-GT-01 and DEL-GT-02-S2 geothermal wells.~\textcite{bruining2023sedimentological} interpreted the cored sections of these wells, translating the raw physical logs into distinct, combined \glspl{fa}. The \glspl{fa} along the wellbore are presented in figure~\ref{fig:appendix_flumy_facies_and_grainsizes}. The specific categorical nature of this well data serves as the link between the synthetic generative models and practical field application, driving two key methodological decisions in this project:
\begin{enumerate}
    ...
    \item The \glspl{fa} of these well-log interpretations, which group the complex subsurface into three primary associations, directly necessitate the subsequent grouping of the generated dataset(s) into upscaled 3-facies dataset(s).
\end{enumerate}
...
To test the \gls{gan} in different scenario', three distinct geological conceptual models were curated to capture a range of environments for the \gls{dsst} reservoir. The setting specific input parameters are shown in Table~\ref{tab:NEXUS_input}.
\begin{table}[htbp]
    \centering
...
The \gls{gan} architecture used in this project is based on the \gls{fluvgan} model, introduced by~\textcite{rongier2025afluvgan}. Where \textcite{rongier2025atowards} used a 2-channel approach for their model, one for grain size and one for deposition time, the model used in this project comprises of either 3 or 9 channels, where each channel represents another facies based on the one-hot encoding strategy. The deposition time channel was not used in this project.
