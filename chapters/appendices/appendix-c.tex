...
The generator network~($\mathbf{G}$) transforms an isotropic Gaussian latent noise vector $\mathbf{z}^* \in \mathbb{R}^{100}$, with $\mathbf{z}^* \sim \mathcal{N}(0, \mathbf{I})$, into a discrete 3D categorical facies realisation. The complete layer configuration is summarized in Table~\ref{tab:best_generator_architecture} and proceeds through six progressive scales:
