\chapter{Mathematical Formulations for Evaluation and Inversion}

\section{Generative Evaluation Metrics}\label{app:evaluation_math}
The evaluation framework relies on advanced statistical divergence and optimal transport metrics to overcome the limitations of standard voxel-by-voxel assessments in generative 3D geomodeling.
...
\begin{equation}\label{eq:entropy_pixel}
    H_{\text{norm}}(\mathbf{v}) = -\frac{1}{\log_2(k)} \sum_{i=1}^{k} P(F_i \mid \mathbf{v}) \log_2 P(F_i \mid \mathbf{v})
\end{equation}
Here, $P(F_i \mid \mathbf{v})$ is the empirical probability of generating facies $F_i$ at the specific voxel $\mathbf{v}$ across the entire ensemble of realisations. An $H_{\text{norm}}(\mathbf{v})$ of $0$ implies total certainty~(uniformity or potential mode collapse at that voxel), while $1$ implies total uncertainty~(maximum diversity across all $k$ facies).
...
\begin{equation}\label{eq:kl_div_pixel}
    D_{KL}(P_{\mathbf{v}} \parallel Q_{\mathbf{v}}) = \sum_{i=1}^{k} P(F_i \mid \mathbf{v}) \log_2\left(\frac{P(F_i \mid \mathbf{v})}{Q(F_i \mid \mathbf{v})}\right)
\end{equation}
...
\end{equation}
Unlike $D_{KL}$, which is asymmetric and can approach infinity if $Q(F_i \mid \mathbf{v})$ is zero, $D_{JS}$ is strictly symmetric ($D_{JS}(P_{\mathbf{v}} \parallel Q_{\mathbf{v}}) = D_{JS}(Q_{\mathbf{v}} \parallel P_{\mathbf{v}})$) and bounded between $[0, 1]$ when calculated with base-2 logarithms, making it a robust final scalar for spatial fidelity.
...
\section{Inversion Framework}\label{app:optimization_math}
The two-step inversion process for the 3-facies generative models is mathematically defined by consecutive optimization phases: Latent Space Optimization followed by Pivotal Tuning.

\subsection{Latent Space Optimization}
During the initial Latent Space Optimization phase of the inversion process, the internal weights of the generator $G$ are frozen. Gradient descent is utilized to iteratively search the latent space $\mathcal{Z}$ for an optimal latent noise vector $\mathbf{z}^*$ that minimizes the loss between the generated facies and the true geological well observations. 

Rather than evaluating the inversion penalty exclusively at the exact well coordinates, a spatially weighted context mask is applied to enforce localized geological alignment without over-constraining the broader, unobserved reservoir architecture. A spatial influence radius is established around each well point, governed by a distance threshold~($\sigma$). This threshold acts as the standard deviation for a continuous Gaussian decay function, dictating the geostatistical \say{reach} of the well data.
...
The inversion objective function is thus defined as a spatially masked Context Loss, replacing the point-wise evaluation. It is evaluated over the grid volume $\mathcal{V}$ and the $C$ total facies classes:
...
\begin{figure}[htbp!]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/appendix/appendix_D/3D_anisotropic_gaussian_mask.png}
...
\textbf{Computational Scaling:} Latent optimization is highly efficient. The computational cost scales linearly, $\mathcal{O}(N_{w} \cdot C)$, with the number of well observation points $N_{w}$ and facies classes $C$. Because the network weights are frozen, the gradient is only computed with respect to the latent vector $\mathbf{z}$~(typically of dimension 512 or 1024), making this phase fast and highly scalable to dense well configurations.

\subsection{Pivotal Tuning}
To overcome the inherent boundaries of the learned latent space and achieve exact match with the observed well data, Pivotal Tuning~\parencite{roich2022pivotal} is applied as the second stage of the inversion framework. Using the previously optimized $\mathbf{z}^*$ as a static anchor, the generator's internal weights $\theta$ are unfrozen and optimized:
